...
try:
    ...
    with open('/var/www/hackatum2019/user_files/' + sys.argv[1], mode="rb") as test_data:
    ...
    for prediction in results.predictions:
        if (prediction.probability > 0.4 ):
            if(prediction.tag_name== 'Studi'):
                img = cv2.imread('/var/www/hackatum2019/user_files/' + sys.argv[1])
                y=img.shape[0]
                x=img.shape[1]
    ...
    f = open("/var/www/hackatum2019/user_files/temp.txt", "wb")
    f.write(json.dumps(Produkte) + "|" + str(Barcode))
    f.close()
